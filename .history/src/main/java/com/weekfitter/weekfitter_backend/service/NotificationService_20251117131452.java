...
public class NotificationService {
    ...
    public void sendPendingNotifications() {
        ...
        for (Notification n : pending) {
            try {
                ...
                // Označení notifikace jako odeslané
                n.setSent(true);
                ...
                // Chyba při odeslání neukončí celý proces – pouze se zaloguje
     ...
     * které odpovídají aktuálně uloženým notifikacím v databázi.
     *
     * Používá se pro:
     * - zobrazení nastavených notifikací ve frontendové aplikaci,
     * - rekonstrukci výběru (např. 5 min, 1 hod, 1 den před začátkem).
     *
     * @param event událost, pro kterou se mají zjistit offsety
     * @return seřazený seznam offsetů v minutách
     */
    public List<Integer> getNotificationOffsetsForEvent(CalendarEvent event) {
        ...
        for (Notification n : notifications) {
            ...
            // Notifikace by měla být před začátkem – záporné nebo nulové hodnoty ignorujeme
            if (minutes > 0) {
                offsets.add((int) minutes);
            }
        }

        // Pro přehlednost seřadíme od nejkratšího k nejdelšímu offsetu
