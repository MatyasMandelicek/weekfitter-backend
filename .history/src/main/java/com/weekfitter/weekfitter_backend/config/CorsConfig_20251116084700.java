...
public class CorsConfig {
    ...
    public CorsFilter corsFilter() {
        ...
        // Povolené HTTP metody používané ve frontendové části aplikace
        config.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "OPTIONS"));

        // Povolené hlavičky v příchozích requestech
        config.setAllowedHeaders(List.of("*"));

        // Povolení přenosu cookies nebo tokenů — důležité, pokud se používá Session nebo JWT v hlavičkách
        config.setAllowCredentials(true);

        // Hlavičky, které mohou být dostupné na frontendové straně po odpovědi serveru
        config.setExposedHeaders(List.of("Authorization", "Content-Type"));

        // Registrace CORS konfigurace pro všechny API endpointy (/**)
