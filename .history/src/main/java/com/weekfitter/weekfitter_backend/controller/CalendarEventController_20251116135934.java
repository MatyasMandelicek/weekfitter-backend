...
public class CalendarEventController {
     ...
     *
     * Identita se určuje pomocí JWT tokenu (principal),
     * nikoliv pomocí parametru e-mail.
     */
    @GetMapping
    public ResponseEntity<?> getEventsByUser(java.security.Principal principal) {

        // Email přihlášeného uživatele je uložen v tokenu
        String email = principal.getName();

        // Ověříme existenci uživatele (centrálně v UserService)
        User user = userService.getUserOrThrow(email);

        // Načtení událostí
        List<CalendarEvent> events = calendarEventService.getEventsByUser(user);

        // Doplnění notifikací – beze změny
