...
// --- uvnitř handleSubmit (změna jen u větve s PUT) ---
...
const url = selectedEvent
  ...
  const handleEventDrop = async ({ event, start, end }) => {
    ...
    // Optimistická aktualizace UI (aby událost "nemizela" než doběhne fetch)
    setEvents((prev) =>
      prev.map((e) =>
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // dovolené: re-sync ze serveru (kdyby BE něco dopočítal)
        await loadEvents();
      }
    } catch (error) {
